...
import { Resource, useCreateResource, useUpdateResource, useDeleteResource, useTogglePin, getListResourcesQueryKey, getGetStatsSummaryQueryKey } from "@workspace/api-client-react";
...
export function ResourceModal({ 
  ...
  const typeColor = resourceTypeColors[formData.type] || "white";

  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[600px] bg-black/40 backdrop-blur-2xl border border-white/10 text-white shadow-2xl p-0 overflow-hidden" style={{ boxShadow: `0 0 40px -10px ${typeColor}20` }}>
          ...
          <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-2">
              ...
              <Input 
                ...
                className="bg-white/5 border-white/10 text-white text-lg placeholder:text-white/20 focus-visible:ring-1 focus-visible:ring-white/20 font-serif"
                placeholder="A magical title..."
              />
            </div>

            {['video', 'website', 'image'].includes(formData.type) && (
              <div className="space-y-2">
                <label className="text-sm text-white/60">URL</label>
                <Input 
                  value={formData.url} 
                  onChange={(e) => setFormData({...formData, url: e.target.value})} 
                  className="bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-1 focus-visible:ring-white/20 font-mono text-sm"
                  placeholder="https://..."
                />
              </div>
            )}
            ...
            {formData.type === 'book' && (
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-1 focus-visible:ring-white/20"
                />
              </div>
            )}
            ...
            {formData.type === 'note' ? (
              <div className="space-y-2">
                ...
                <Textarea 
                  ...
                  className="bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-1 focus-visible:ring-white/20 min-h-[150px] font-sans"
              ...
              <div className="space-y-2">
                ...
                <Textarea 
                  ...
                  className="bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-1 focus-visible:ring-white/20 min-h-[80px]"
                />
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <Input 
                ...
                className="bg-white/5 border-white/10 text-white placeholder:text-white/20 focus-visible:ring-1 focus-visible:ring-white/20"
                placeholder="inspiration, tutorial, reference"
              />
            </div>
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                {[
                  ...
                  <button
                    key={c.value}
                    type="button"
                    onClick={() => setFormData({...formData, colorLabel: c.value})}
                    className={cn(
                      "w-6 h-6 rounded-full transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-black",
                      formData.colorLabel === c.value ? "scale-125 ring-2 ring-white" : ""
                    )}
                ...
                <button
                  type="button"
                  onClick={() => setFormData({...formData, colorLabel: undefined})}
                  className={cn(
                    "w-6 h-6 rounded-full border border-white/20 flex items-center justify-center text-white/50 hover:bg-white/10 hover:text-white transition-colors",
                    !formData.colorLabel ? "ring-2 ring-white" : ""
                  )}
                >
